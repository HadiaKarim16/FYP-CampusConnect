...
export default function EventDashboard() {
    ...
    return (
      <div className="flex h-screen w-full items-center justify-center bg-[#112118] text-[#1dc964]">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-[#9eb7a9] animate-pulse">Loading event details...</p>
        </div>
      </div>
    );
  }
  ...
  if (error || (!event && eventId)) {
    return (
      <div className="flex h-screen w-full flex-col items-center justify-center bg-[#112118] text-white p-6 text-center">
        ...
        <p className="text-[#9eb7a9] mb-6 max-w-md">{error || "The event you're looking for doesn't exist or is no longer available."}</p>
        <button onClick={() => navigate(-1)} className="px-6 py-2 bg-[#1dc964] text-[#112118] font-bold rounded-lg hover:opacity-90">
          Go Back
        </button>
      </div>
    );
  ...
  if (!eventId) {
    return (
      <div className="flex h-screen w-full flex-col items-center justify-center bg-[#112118] text-white p-6 text-center">
        ...
        <p className="text-[#9eb7a9] mb-6 max-w-md">Select an event from your list to view detailed analytics and manage participants.</p>
        <button onClick={() => navigate("/mentor-events")} className="px-6 py-2 bg-[#1dc964] text-[#112118] font-bold rounded-lg hover:opacity-90">
          Browse Events
        </button>
      </div>
    );
  ...
  return (
    <div className="relative flex min-h-screen w-full flex-col font-display text-[#c9d1d9] bg-[#112118] overflow-x-hidden">
      <MentorTopBar backPath="/mentor-events" />
      
      <main className="px-4 sm:px-6 lg:px-8 xl:px-10 py-8 max-w-6xl mx-auto w-full">
        {/* Header Section */}
        <div className="relative rounded-3xl overflow-hidden bg-[#161b22] border border-[#30363d] mb-8 shadow-2xl">
          {event.coverImage ? (
          ...
          <div className="p-8 -mt-20 relative z-10">
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
              <div className="flex-1">
                ...
                <div className="flex flex-wrap gap-4 text-[#9eb7a9] text-sm">
                  <div className="flex items-center gap-2 bg-[#0d1117]/50 px-3 py-1.5 rounded-full border border-[#30363d]">
              ...
              <div className="flex gap-3">
                <button className="px-6 py-3 bg-[#1dc964] text-[#112118] font-black rounded-xl hover:scale-105 transition-transform shadow-lg shadow-[#1dc96433]">
                  Edit Event
                </button>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 flex flex-col gap-8">
            <section className="bg-[#161b22] border border-[#30363d] rounded-2xl p-6">
              ...
              <p className="text-[#9eb7a9] leading-relaxed whitespace-pre-wrap">
            ...
            <section className="bg-[#161b22] border border-[#30363d] rounded-2xl p-6">
              ...
              {event.teamCount > 0 ? (
                ...
                <div className="flex flex-col items-center justify-center py-12 text-[#9eb7a9] border-2 border-dashed border-[#30363d] rounded-xl">
          ...
          <div className="flex flex-col gap-8">
            <section className="bg-[#161b22] border border-[#30363d] rounded-2xl p-6">
              ...
              <div className="flex flex-col gap-4">
                <div className="flex justify-between items-start py-2 border-b border-[#30363d]/50">
                  <span className="text-[#9eb7a9] text-sm font-medium">Organizer</span>
                  <span className="text-white text-sm font-semibold">{event.societyId?.name || "Campus Society"}</span>
                </div>
                <div className="flex justify-between items-start py-2 border-b border-[#30363d]/50">
                  <span className="text-[#9eb7a9] text-sm font-medium">Status</span>
                  <span className="text-[#1dc964] text-xs font-bold uppercase">{event.status}</span>
                </div>
                <div className="flex justify-between items-start py-2 border-b border-[#30363d]/50">
                  <span className="text-[#9eb7a9] text-sm font-medium">Category</span>
                  <span className="text-white text-sm font-semibold">{event.category || "General"}</span>
                </div>
                <div className="flex justify-between items-start py-2">
                  <span className="text-[#9eb7a9] text-sm font-medium">Type</span>
            ...
            <section className="bg-gradient-to-br from-[#1dc96411] to-transparent border border-[#1dc96433] rounded-2xl p-6">
              ...
              <p className="text-[#9eb7a9] text-xs mb-4 leading-normal">
                Registration for this event is currently active. Participants can join the event through the campus portal.
              </p>
              <div className="p-3 bg-[#0d1117] rounded-xl border border-[#1dc96422] flex items-center justify-between">
                <span className="text-[#1dc964] font-bold text-lg">{event.teamCount || 0}</span>
                <span className="text-[#9eb7a9] text-xs uppercase font-black">Active Registrations</span>
              </div>
            </section>
          </div>
        </div>
